...
//stage 4 변수
let hiddenCanvas; //숨겨진 캔버스
let capture;
let capturedImage; //캡쳐한 이미지 저장할 변수
let captureTime;
...
function setup() {
  ...
  */
  ...
  // stage 4
  hiddenCanvas = createGraphics(1000, 600);
  capture = createCapture(VIDEO);
  capture.size(350, 350);
  capture.hide();
  tageakSound.playMode('sustain'); //타격통보 소리 더 부드럽게 함
...
function mouseClicked() {
  ...
  if (state === 'stage2') {
    ...
    if (
      mouseX >= invenObjX1 &&
      mouseX <= invenObjX1 + invenObjW1 &&
      mouseY >= invenObjY1 &&
      mouseY <= invenObjY1 + invenObjH1 &&
      invenArr[1] == boxGo
    ) {
      boxClick = !boxClick;
      return;
    ...
    if (
      mouseX >= invenObjX2 &&
      mouseX <= invenObjX2 + invenObjW2 &&
      mouseY >= invenObjY2 &&
      mouseY <= invenObjY2 + invenObjH2 &&
      invenArr[1] == freeGo
    ) {
      freeClick = !freeClick;
      if (freeClick) {
    ...
    if (
      mouseX >= invenObjX3 &&
      mouseX <= invenObjX3 + invenObjW3 &&
      mouseY >= invenObjY3 &&
      mouseY <= invenObjY3 + invenObjH3 &&
      invenArr[2] == imhoGo
    ) {
      imhoClick = !imhoClick;
      if (imhoClick) {
    ...
    if (
      mouseX >= invenObjX5 &&
      mouseX <= invenObjX5 + invenObjW5 &&
      mouseY >= invenObjY5 &&
      mouseY <= invenObjY5 + invenObjH5 &&
      invenArr[3] == haedokGo
    ) {
...
function keyPressed() {
  ...
  if (state === 'mission3' && keyCode === ENTER) {
    ...
    if (userInput === stage3correctAnswer) {
      inputBox.hide(); // 입력 창 숨기기
  ...
  // stage 4
  if (state === 'cardcam' && key === ' ') {
    camSound.play(); // 카메라 사운드 재생
    // 캡처 및 이미지 저장 로직은 기존 코드와 동일
    hiddenCanvas.background(card);
    hiddenCanvas.image(capture, 350, 200, 350, 350);
    hiddenCanvas.filter(GRAY);

    hiddenCanvas.save('myCard.jpg');
    capturedImage = createImage(350, 350);
    capturedImage.copy(
      capture,
      0,
      0,
...
function displayStage1() {
  ...
  if (currentSentenceIndex < 5) {
    ...
    let kimkooText = [
      '1919년 2월 최남선이 작성한 독립선언서가 인쇄소로 넘겨졌소.',
...
function displayStage2() {
  ...
  if (bosungStartMent && state === 'stage2') {
    ...
    textSize(textS);
    textAlign(CORNER);
    text('인쇄된 독립 선언서를 찾아서 전달하자.', textX, textY); //시작 멘트
    text(
      '독립 선언서는 어딘가에 숨겨져 보관되어 있을 텐데..',
      textX,
      textY + textS
    ); //시작 멘트
  ...
  if (imhoClick) {
    ...
    textSize(textS);
    textAlign(CORNER);
    text('해독표가 있어야 암호를 해독할 수 있을 것 같은데..', textX, textY); //자물쇠 대사
...
function Stage3readMission() {
  ...
  if (!speechReady && speech === '') {
    // 음성 인식 대기 상태
    text('화면을 마우스로 클릭하세요.', width / 2, height / 2);
  } else if (speechReady) {
    // 음성 인식 진행 중
    text('독립운동의 구호를 크게 외쳐보세요.', width / 2, height / 2);
...
function displayCardcam() {
  background(card);
  image(capture, 350, 200, 350, 350);
  filter(GRAY);

  if (millis() - captureTime > 2000) {
    image(capturedImage, 350, 200, 350, 350);
    state = 'heungmu';
  }
}
